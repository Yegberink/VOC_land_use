...
#%%
tokened_sentences = pd.read_csv("output/datasets/NER_tokenedsentences_ceylon.csv", sep=';')
...
for i in range(len(ceylon_documents_clean)):
    ...
    k = i + 20607 #Add a number to make it possible to pause and continue
    print(k) # to keep track of the progress
    
    # Extract the question from the second column
    content = ceylon_documents_clean.iloc[k, 1]
    ...
    ceylon_documents_clean.at[k, "NamedEntities"] = locations_string #Save data
...
#%% Save the results
tokened_sentences.to_csv("output/datasets/NER_tokenedsentences_ceylon.csv", index=False, sep=';')
...
interesting_document_numbers_ceylon = set(pd.read_table("Data/interesting_document_numbers_ceylon.txt", header=None)[0])

#Filter for these numbers
interesting_documents_ceylon = files_df[files_df["LabelIdentifier"].isin(interesting_document_numbers_ceylon)].reset_index(drop=True)

#%%Save the dataset for checking
interesting_documents_ceylon.to_csv("output/datasets/interesting_documents_ceylon.csv", index=False, sep=';')
